...
const UnitInfoDetails = () => {
  ...
  const { hasChanges } = useSelector(getCourseUnitData);
  ...
  // After a lifecycle publish, re-fetch the unit's vertical data so PublishControls
  // (editedOn, publishedOn, discard button) and the hasChanges prop reflect the new state.
  ...
  return (
    <SidebarContent>
      <PublishControls blockId={blockId} hideCopyButton hidePublishButton />
      <LifecycleSection usageKey={blockId} title="Unit Status" hasChanges={hasChanges} onPublishSuccess={handlePublishSuccess} />
      <SidebarSection
